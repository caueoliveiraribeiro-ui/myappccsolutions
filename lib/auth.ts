...
export function createSession(email:string,id=OWNER_ID,mfaVerified=false){const expiry=Date.now()+1000*60*60*12,payload=Buffer.from(JSON.stringify({id,email,expiry,mfaVerified})).toString("base64url"),signature=createHmac("sha256",secret()).update(payload).digest("base64url");return `${payload}.${signature}`}
