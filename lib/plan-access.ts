...
async function directAccess(id:string):Promise<Access> {
...
export async function accountAccess(id:string):Promise<Access> {
  const own=await directAccess(id)
  if(own.plan!=="none")return own
  try {
    const memberships=await db(`workspace_members?member_user_id=eq.${encodeURIComponent(id)}&select=owner_user_id&limit=20`)
    for(const membership of memberships||[]){
      const owner=await directAccess(String(membership.owner_user_id||""))
      if(owner.plan!=="none")return {...owner,status:"shared",accessUntil:owner.accessUntil||null}
    }
  }catch{}
  return own
}
