...
function subscriptionPlan(subscription: Stripe.Subscription) {
  ...
  return plan && isSubscriptionPlan(plan) ? plan : null
}
...
async function bindSubscription(args: {
  ...
  return db("rpc/orbit_stripe_bind_subscription", {
    method: "POST",
    body: JSON.stringify({
      p_user: args.userId,
...
async function applySubscription(args: {
  ...
  return db("rpc/orbit_apply_stripe_event", {
    method: "POST",
    body: JSON.stringify({
...
async function handleCheckoutCompleted(event: Stripe.Event, session: Stripe.Checkout.Session) {
  if (session.mode !== "subscription") return { ignored: true }

  const subscriptionId = stripeId(session.subscription, "sub_")
  const customerId = stripeId(session.customer, "cus_")
  if (!subscriptionId || !customerId) throw new Error("INVALID_STRIPE_CHECKOUT_BINDING")

  const email = session.customer_details?.email?.trim().toLowerCase() || session.customer_email?.trim().toLowerCase()
  ...
  if (!plan) throw new Error("INVALID_STRIPE_SUBSCRIPTION_PLAN")

  const observed = new Date(event.created * 1000).toISOString()
  let userId = typeof session.metadata?.orbit_user_id === "string" ? session.metadata.orbit_user_id : null
  let created = false
  let setupToken: string | null = null

  if (userId) {
    const rows = await db(`app_users?id=eq.${encodeURIComponent(userId)}&select=id,email&limit=1`)
    const existing = rows?.[0]
    if (!existing) throw new Error("ORBIT_USER_NOT_FOUND")
    if (String(existing.email).trim().toLowerCase() !== email) throw new Error("ORBIT_EMAIL_MISMATCH")
  } else {
  ...
  return {
    ...
    userId,
    created,
    setupEmailSent: Boolean(created && setupToken),
  }
}
...
async function handleSubscriptionEvent(
  ...
  if (!customerId) throw new Error("INVALID_STRIPE_CUSTOMER")

  const metadataUserId = subscription.metadata?.orbit_user_id
  let userId = typeof metadataUserId === "string" && metadataUserId ? metadataUserId : null

  if (!userId) {
    const rows = await db(`orbit_stripe_subscriptions?subscription_id=eq.${encodeURIComponent(subscriptionId)}&select=user_id&limit=1`)
    userId = rows?.[0]?.user_id || null
  }

  if (!userId) throw new Error("RETRY_UNBOUND_STRIPE_SUBSCRIPTION")

  return applySubscription({
    eventId: event.id,
    observed: new Date(event.created * 1000).toISOString(),
    userId,
    subscriptionId,
    customerId,
    plan,
    status: subscription.status,
    paidUntil: paid ? paidUntil(subscription) : null,
    paid,
  })
}
...
export async function processStripeEvent(event: Stripe.Event) {
  switch (event.type) {
    ...
    case "customer.subscription.deleted":
      return handleSubscriptionEvent(event, event.data.object as Stripe.Subscription, false)
