    ...
    <Card className="overflow-hidden border-cyan-300/25 bg-[linear-gradient(135deg,rgba(7,17,31,.98),rgba(14,32,52,.96))] p-0 text-white shadow-[0_18px_65px_rgba(0,0,0,.22)]">
      <details className="group" open>
        ...
        <form className="grid gap-4 border-t border-cyan-300/15 bg-black/10 p-5 sm:p-6 md:grid-cols-2" onSubmit={createInvoice}>
          ...
          <label className="text-xs font-medium text-slate-300">Client email<Input key={`email-${selectedClient}`} name="client_email" type="email" defaultValue={client?.email || ""} className="mt-1.5 h-11"/></label><label className="text-xs font-medium text-slate-300">Company name / invoice brand<Input key={`company-${selectedClient}`} name="client_company_name" defaultValue={client?.company_name || ""} placeholder="Company shown on the PDF" className="mt-1.5 h-11"/></label><label className="text-xs font-medium text-slate-300 md:col-span-2">Billing address<Textarea key={`address-${selectedClient}`} name="client_address" defaultValue={client?.address || ""} className="mt-1.5 min-h-20" placeholder="Street, suite, city, region, postal code, country"/></label>
