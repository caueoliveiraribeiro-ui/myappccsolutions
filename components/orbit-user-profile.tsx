...
export function OrbitUserProfile() {
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      if (!triggerRef.current?.isConnected) bindTrigger()
    })
  ...
  useEffect(() => {
    ...
    const close = (event: KeyboardEvent) => {
      ...
      const nodes = Array.from(dialogRef.current?.querySelectorAll<HTMLElement>('button, input, select, textarea, [tabindex="0"]') || []).filter(node => node.offsetParent !== null && !node.hasAttribute("disabled"))
  ...
  return (
    <div ref={dialogRef} className="fixed inset-0 z-[150] overflow-y-auto overscroll-contain bg-black/80 p-2 pb-[max(1rem,env(safe-area-inset-bottom))] backdrop-blur-md sm:p-6" role="dialog" aria-modal="true" aria-label="Orbit user profile">
      <div className="mx-auto w-full max-w-5xl overflow-hidden rounded-[30px] border border-cyan-300/20 bg-[#06101c] text-white shadow-[0_30px_120px_rgba(0,0,0,.7)]">
        <div className="relative flex items-start justify-between gap-2 border-b border-white/10 bg-gradient-to-r from-cyan-300/[.08] to-violet-300/[.06] p-5 sm:p-7">
          <div className="flex min-w-0 flex-1 flex-col items-start gap-3 pr-10 sm:flex-row sm:items-center sm:gap-4">
            <label className="group relative grid h-16 w-16 shrink-0 cursor-pointer sm:h-20 sm:w-20 place-items-center overflow-hidden rounded-2xl border border-cyan-300/25 bg-gradient-to-br from-cyan-300 to-violet-300 text-2xl font-bold text-slate-950">
              ...
              <span className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 bg-black/65 py-1 text-[10px] font-semibold text-white opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100"><Camera size={11}/> Change</span>
            ...
            <div>
              ...
              <h2 className="mt-1 break-words text-xl font-semibold sm:text-2xl">{profile?.name || "Your account"}</h2>
          ...
          <button type="button" onClick={() => setOpen(false)} className="absolute right-3 top-3 grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-white/10 bg-black/20 sm:right-5 sm:top-5 hover:bg-white/10" aria-label="Close profile"><X size={19}/></button>
        </div>
        ...
        <div className="grid gap-5 p-4 sm:p-6 xl:grid-cols-[1.5fr_.85fr]">
          <form onSubmit={saveProfile} className="space-y-5">
            <section className="rounded-2xl border border-white/10 bg-white/[.025] p-4 sm:p-5">
              ...
              <label className="mt-3 block text-xs text-slate-400">About you<textarea name="bio" maxLength={500} defaultValue={profile?.bio || ""} rows={3} className="mt-1 w-full resize-y rounded-xl border border-white/10 bg-[#091522] px-3 py-2 text-base sm:text-sm text-white outline-none focus:border-cyan-300/40" placeholder="A short note about your role, priorities or workspace."/></label>
            </section>
            ...
            <section className="rounded-2xl border border-white/10 bg-white/[.025] p-4 sm:p-5">
              ...
              <div className="grid min-w-0 gap-3 sm:grid-cols-2">
                ...
                <label className="text-xs text-slate-400">Language<select name="language" defaultValue={profile?.language || "en"} className="mt-1 h-11 min-w-0 w-full rounded-xl border border-white/10 bg-[#091522] px-3 text-base sm:text-sm">{languages.map(([code,label])=><option key={code} value={code}>{label}</option>)}</select></label>
                <div className="sm:col-span-2 rounded-xl border border-cyan-300/15 bg-cyan-300/[.025] p-3">
                  ...
                  <div className="grid min-w-0 gap-3 sm:grid-cols-2">
                    <label className="text-xs text-slate-400">Country<select name="country" value={selectedCountry} onChange={(event) => setSelectedCountry(event.target.value)} className="mt-1 h-11 min-w-0 w-full rounded-xl border border-white/10 bg-[#091522] px-3 text-base sm:text-sm">{countries.map(([code,label])=><option key={code} value={code}>{label}</option>)}</select></label>
                    <label className="text-xs text-slate-400">Default currency<select name="currency" value={selectedCurrency} onChange={(event) => setSelectedCurrency(event.target.value)} className="mt-1 h-11 min-w-0 w-full rounded-xl border border-white/10 bg-[#091522] px-3 text-base sm:text-sm">{currencies.map((code)=><option key={code}>{code}</option>)}</select></label>
                  </div>
          ...
          <div className="space-y-5">
            <form onSubmit={changePassword} className="rounded-2xl border border-white/10 bg-white/[.025] p-4 sm:p-5">
              ...
              <button type="submit" disabled={passwordBusy} className="mt-4 h-11 min-w-0 w-full rounded-xl border border-cyan-300/25 bg-cyan-300/[.08] px-4 text-sm font-semibold text-cyan-100 hover:bg-cyan-300/[.14] disabled:opacity-60">{passwordBusy ? "Updating…" : "Change password"}</button>
            </form>
...
function Field({ label, ...props }: { label: string; [key: string]: any }) {
  return <label className="text-xs text-slate-400">{label}<input {...props} className="mt-1 h-11 min-w-0 w-full rounded-xl border border-white/10 bg-[#091522] px-3 text-base sm:text-sm text-white outline-none focus:border-cyan-300/40 disabled:cursor-not-allowed disabled:opacity-60"/></label>
}

function Info({ label, value }: { label: string; value: any }) {
  return <div className="flex items-center justify-between gap-3 border-b border-white/[.06] pb-2"><span className="text-slate-500">{label}</span><span className="max-w-[60%] break-all text-right capitalize text-slate-200">{String(value || "—")}</span></div>
}
