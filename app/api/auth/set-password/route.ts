...
export async function POST(request: Request) {
  ...
  if (origin !== APP_ORIGIN && origin !== requestOrigin) {
    return NextResponse.json(
      { error: "Please set your password from the Orbit website." },
      { status: 403 }
    )
  }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Use a valid setup link and a password between 12 and 128 characters.",
        },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      {
        error:
          "Your password could not be saved right now. Please try again.",
      },
      { status: 503 }
    )
  }
