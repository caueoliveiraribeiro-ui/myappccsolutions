...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (raw.length > 4096) return NextResponse.json({ error: "Invalid password reset request." }, { status: 400 })
    ...
    const changed = await db("rpc/orbit_password_reset_consume", {
      method: "POST",
      body: JSON.stringify({
    ...
    response.cookies.set("orbit_session", "", { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "strict", path: "/", maxAge: 0 })
    return response
  } catch (error) {
    console.error("ORBIT_PASSWORD_RESET_FAILED:", error)
    return NextResponse.json({ error: "Your password could not be reset right now. Please try again." }, { status: 503 })
  }
}
