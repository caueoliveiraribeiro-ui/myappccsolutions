...
function cleanFields(input:Record<string,unknown>){const out={...input};delete out.deleted_at;delete out.delete_after;delete out.email_claim_token;delete out.email_claimed_at;delete out.email_provider_id;delete out.email_delivery_error;const dates=new Set(["due_date","deadline","payment_date","received_at","charge_date","last_call_date","next_follow_up_date","next_follow_up","expense_date","purchased_at","start_time","consumed_at","target_date","issue_date","sent_at"]);const nullableLinks=new Set(["client_id","project_id","portfolio_id","lead_id","source_lead_id","source_client_id","billing_client_id","source_project_id"]);const numbers=new Set(["amount","budget","cost","estimated_value","service_amount","lifetime_value","quantity","estimated_cost","actual_cost","buy_price","current_price","grams","calories","target_amount","current_amount"]);for(const [key,value] of Object.entries(out)){if(value===""&&(dates.has(key)||nullableLinks.has(key)))out[key]=null;if(numbers.has(key)&&value==="")out[key]=0;if((key==="currency"||key==="quote_currency")&&typeof value==="string")out[key]=value.trim().toUpperCase()}return out}
