...
export async function GET(request:Request){
 ...
 try{
  if(type==="stock"){
   ...
   return NextResponse.json({symbol,name:symbol,price:Number(q["05. price"]),currency:"USD",source:"Alpha Vantage"});
  }
  ...
  const r=await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${encodeURIComponent(id)}&vs_currencies=usd`,{headers,cache:"no-store"}),d=await r.json();
  if(!d[id]?.usd)throw Error("Crypto symbol not found. Try BTC, ETH, SOL or a CoinGecko coin ID.");
  return NextResponse.json({symbol,name:id,price:Number(d[id].usd),currency:"USD",source:"CoinGecko"});
