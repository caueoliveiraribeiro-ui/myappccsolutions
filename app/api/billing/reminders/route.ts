...
export async function POST(){const planDenied=await requestFeature("reports");if(planDenied)return planDenied;
 ...
 try{
   ...
   for(const owner of owners){if(!(await accountAccess(owner)).features.includes("reports"))continue;await db("rpc/orbit_create_billing_reminders",{method:"POST",body:JSON.stringify({p_owner:owner})});await db("rpc/orbit_create_upcoming_payments",{method:"POST",body:JSON.stringify({p_owner:owner})});await db("rpc/orbit_create_invoice_drafts",{method:"POST",body:JSON.stringify({p_owner:owner})});}
